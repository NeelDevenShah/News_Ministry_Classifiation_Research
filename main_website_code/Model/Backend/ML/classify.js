...
const Url_sent = "http://172.16.102.248:7203/sentiment/get-sentiment";
const Url_min = "http://172.16.102.248:7203/ministry/get-ministry-class";
  ...
  Promise.all(promises).then(async (response) => {
   ...
   // console.log(response[1].data[1].ministries);
    ...
    for (let index = 0; index < news_arr.length; index++) {
      ...
      element.sentiment = response[0].data[index].sentiment;
      element.ministry = ministry_list[response[1].data[index].ministries[0]];
